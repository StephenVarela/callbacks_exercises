...
*/
var numSales = countSales(transactions); //function takes an array of objects

function countSales(trans){
  count = 0;
...
*/
var numPurchases = countPurchases(transactions);


function countPurchases(trans){
  count = 0;
  for(let i = 0; i < trans.length; i++){
...
*/
var numCashSales = countTypePurchases(transactions, 'cash');


function countTypePurchases(trans, type){
  count = 0;
  for(let i = 0; i < trans.length; i++){
    if(trans[i].type === "purchase" && trans[i].paymentMethod === type){
      count++;
    }
  }
  return count;
...
*/
var numCreditPurchases = countTypePurchases(transactions, 'credit');
...
*/
var allVendors = listVendors(transactions);

function listVendors(trans){
  let vendors = []
  for(let i = 0; i < trans.length; i++){
    var includes = false;
    if(trans[i].vendor){
      vendors.forEach(function(vendor){
        if(vendor === trans[i].vendor){
          includes = true
        }
      })
      if(includes === false){
        vendors.push(trans[i].vendor)
      }
    }
  }
  return vendors;
...
*/
var uniqueCustomers = listUniqueCustomers(transactions);


function listUniqueCustomers(trans){
  let customers = []
  for(let i = 0; i < trans.length; i++){
    var includes = false;
    if(trans[i].customer){
      customers.forEach(function(customer){
        if(customer === trans[i].customer){
          includes = true
        }
      })
      if(includes === false){
        customers.push(trans[i].customer)
      }
    }
  }
  return customers;
...
*/
var bigSpenders = findBigSpenders(transactions);

function findBigSpenders(trans){
  let spenders = []
  for(let i = 0; i < trans.length; i++){
    if(trans[i].type === "sale" && trans[i].items.length >= 5 ){
      spenders.push({"name": trans[i].customer, "numItems": trans[i].items.length})
    }
  }
  return spenders;
}
...
*/
var mostItems = mostItemsInSale(bigSpenders);

function mostItemsInSale(bigSpenders){
  let max = 0
  bigSpenders.forEach(function(spender){

    if(spender.numItems > max){
      max = spender.numItems;
    }
...
*/
var sumOfSmallestPurchase = smallestPurchaseList(transactions);

function smallestPurchaseList(trans){
  let min = 0
  for(let i = 0; i < trans.length; i++){
    if(trans[i].type === "purchase"){
      if(min === 0 ){
        min = trans[i].items.length
      }else if(trans[i].items.length < min){
        min = trans[i].items.length;
      }
    }
  }
